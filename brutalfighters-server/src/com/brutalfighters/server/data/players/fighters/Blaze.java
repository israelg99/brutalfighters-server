...
public class Blaze extends Fighter {
	...
	public Blaze(Connection connection, Base base, String m_id) {
		super(connection, base, m_id, "blaze", 1000, 1000, new Vec2(90,100), 10, //$NON-NLS-1$
	...
	public void skill4() {
		...
		if(getPlayer().getSkillCD()[3] > 0) {
			if(getPlayer().getSkillCD()[3] / GameServer.getDelay() % 2 == 0) {
				...
				GameMatchManager.getCurrentProjectiles().add(new RedEnergyBall(this, getPlayer().getFlip(), new Vec2(xstart, getPlayer().getPos().getY()+MathUtil.nextInt(-50, 10)), 30, 0, new Buff[] {new Buff_Burn(2)}));
			}
